...
function TestimonialCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "bg-white rounded-2xl p-5 sm:p-6 border border-border shadow-[0_2px_16px_rgba(30,90,152,0.06)]",
        "flex flex-col h-full min-h-[220px] max-h-[300px] overflow-hidden cursor-pointer relative z-20",
        "transition-[border-color] duration-200 hover:border-primary-light/30",
        className
      )}
    >
      <p className="text-sm sm:text-base text-ink leading-relaxed flex-1 min-h-0 line-clamp-5 overflow-hidden break-words">
        &ldquo;{testimonial.quote}&rdquo;
      </p>

      <div className="flex items-center gap-3 mt-4 pt-4 border-t border-border shrink-0 min-w-0">
        <div
          className={cn(
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm sm:text-base font-heading font-semibold text-ink truncate">
            {testimonial.name}
          </p>
    ...
    >
      <div className="relative max-w-[1200px] mx-auto">
        ...
        <div className="flex justify-between gap-4 xl:gap-6">
          <div className="w-[min(100%,320px)] xl:w-[360px] flex flex-col gap-5 xl:gap-6 shrink-0">
            <AnimatePresence mode="wait">
              {left.map((t, i) => (
                <motion.div
                  key={`${page}-l-${i}`}
          ...
          <div className="w-[min(100%,320px)] xl:w-[360px] flex flex-col gap-5 xl:gap-6 shrink-0 mt-10 xl:mt-12">
            <AnimatePresence mode="wait">
              {right.map((t, i) => (
                <motion.div
                  key={`${page}-r-${i}`}
